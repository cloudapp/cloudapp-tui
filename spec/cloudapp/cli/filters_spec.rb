...
describe CloudApp::TUI::Filters do
  subject { CloudApp::TUI::Filters.new }
  ...
  describe '#selection_index' do
    ...
    it 'returns the given selected index' do
      filters = CloudApp::TUI::Filters.new selection_index: 1
      filters.selection_index.should eq(1)
    end

    it 'maxes out at 2' do
      filters = CloudApp::TUI::Filters.new selection_index: 42
  ...
  describe '#selection' do
    it 'returns the selected filter' do
      filters = CloudApp::TUI::Filters.new selection_index: 0
      filters.selection.should eq('active')

      filters = CloudApp::TUI::Filters.new selection_index: 1
      filters.selection.should eq('trash')

      filters = CloudApp::TUI::Filters.new selection_index: 2
      filters.selection.should eq('all')
    end
  end

  describe '#previous_selection' do
    subject { CloudApp::TUI::Filters.new(selection_index: 1).previous_selection }

    it { should be_a(CloudApp::TUI::Filters) }
    its(:selection_index) { should eq(0) }
  end

  describe '#next_selection' do
    subject { CloudApp::TUI::Filters.new(selection_index: 0).next_selection }
